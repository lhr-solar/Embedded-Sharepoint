...
/**
 * @brief Board/app-provided GPIO bring-up & teardown for a UART instance.
 *
 * Called by HAL_UART_MspInit()/HAL_UART_MspDeInit() after clock/NVIC setup. The
 * project's board file ships a __weak default pin map (HAL_UART_MspGPIOInit; see
 * test/bsp/ for an example); a board or test may override it (strong) to remap pins.
 */
void HAL_UART_MspGPIOInit(UART_HandleTypeDef* handle);
void HAL_UART_MspGPIODeInit(UART_HandleTypeDef* handle);
