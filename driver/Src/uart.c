...
/*
 * HAL_UART_MspGPIOInit() (the per-instance pin map) is provided by the project's
 * board file as a __weak default (see test/bsp/ for an example) and may be
 * overridden (strong) by a board or test to remap pins. Its prototype is in
 * uart.h; HAL_UART_MspInit() below calls it.
 */
