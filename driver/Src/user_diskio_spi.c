...
#include "ff_gen_drv.h"  // Needed for FatFs diskio definitions

extern SPI_HandleTypeDef hspi2;  // SPI used for SD card
...
#define FCLK_SLOW() { MODIFY_REG(SD_SPI_HANDLE.Instance->CR1, SPI_BAUDRATEPRESCALER_256, SPI_BAUDRATEPRESCALER_128); }
...
#include "sdcard.h"  // your low-level SD functions (SD_Init, SD_ReadSector, SD_WriteSector)

static volatile DSTATUS Stat = STA_NOINIT;

DSTATUS USER_SPI_initialize(BYTE pdrv) {
    if (pdrv != 0) return STA_NOINIT;
    if (SD_Init(&hsd) == 0) { // hsd is your sd_handle_t struct
        Stat &= ~STA_NOINIT;
        return Stat;
    } else {
        Stat = STA_NOINIT;
...
DRESULT USER_SPI_read(BYTE pdrv, BYTE *buff, DWORD sector, UINT count) {
    ...
    for (UINT i = 0; i < count; i++) {
        if (SD_ReadSector(&hsd, sector + i, buff + i*512) != 0) return RES_ERROR;
    }
    return RES_OK;
}
...
DRESULT USER_SPI_write(BYTE pdrv, const BYTE *buff, DWORD sector, UINT count) {
    if (pdrv != 0 || count == 0) return RES_PARERR;
    for (UINT i = 0; i < count; i++) {
        if (SD_WriteSector(&hsd, sector + i, buff + i*512) != 0) return RES_ERROR;
    }
    return RES_OK;
}
#endif
...
DRESULT USER_SPI_ioctl(BYTE pdrv, BYTE cmd, void *buff) {
    return RES_OK;  // simple stub; extend if needed
}
#endif
