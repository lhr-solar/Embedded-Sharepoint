...
/* The queu is to be created to hold a mximum of 128 UARTMessages */
...
HAL_StatusTypeDef BSP_UART_Init(UART_HandleTypeDef *huart, QueueHandle_t *rxQueue) { 
    ...
    status = HAL_UART_REGISTER_CALLBACKS(huart, HAL_UART_ERROR_CB_ID, HAL_UART_ErrorCallback);
    if (status != HAL_OK) {
        return status;
    }
    ...
    HAL_NVIC_SetPriority(USART1_IRQn, 5, 0); // set to priority 5 (not the highest priority) for USART1 peripheral's interrupt
    HAL_NVIC_EnableIRQ(USART1_IRQn); // Enable the UART1 interrupt
...
HAL_StatusTypeDef BSP_UART_Write(char *data, uint8_t length, uint8_t busId) {
    ...
    if (HAL_UART_GetState(huartPtr) == HAL_UART_STATE_READY && uxQueueMessagesWaiting(xTxQueue)) {
