...
# For example:
make TEST=blinky PROJECT_TARGET=stm32g491vet
# This compiles the blinky_test.c test for the STM32G491
```
...
1. A directory under `stm/stm32g4xx` must be generated with the generic name of the microcontroller. This is typically `stm32` followed by four characters specifying the series and line (e.g. `stm32g491`, `stm32g431`).
2. A linker script under the aforementioned directory. This can be autogenerated by STM32CubeMX or grabbed from the internet. The name should take the form of `STM32*_FLASH.ld`, and should include the full name of the processor (e.g. `STM32G491VETx_FLASH.ld`, `STM32G431CBTx_FLASH.ld`).
3. A startup assembly file including the vector table and some basic handlers (most important being Reset_Handler). This can be autogenerated by STM32CubeMX or grabbed from the internet. The name should take the form of `startup_stm32` followed by the four characters specifying the series and line and then `xx` (e.g. `startup_stm32g491xx.s`, `startup_stm32g431xx.s`).
4. A configuration file with the exact MCU name (e.g. `stm32g431cbt.cfg`) for any per-target Embedded-Sharepoint feature flags. It may be empty; the build includes it as a hook for future options.
...
Board bring-up (pins, peripheral instances, and the system clock if it uses an external crystal) is owned by the project that submodules Embedded-Sharepoint, not by the library. Copy `test/bsp/` as a starting point for your own `bsp/` board layer.
