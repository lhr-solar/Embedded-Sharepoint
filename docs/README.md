...
4. A configuration file with the exact MCU name (e.g. stm32l432kcu.cfg) specifying which BSP modules to enable in a space separated list like so:
    `BSP_ENABLE=CAN UART ADC`
    Please check which peripherals actually exist on your MCU and enable only those, or be prepared for strange build errors.
