...
/*
 * ponytail: FDCAN pin map + bit timing are lifted from the legacy defaults;
 * verify against the STM32G491VET board schematic before relying on them.
 *   FDCAN1 PA11/PA12 (AF9), FDCAN2 PB12/PB13 (AF9), FDCAN3 PA8/PA15 (AF11).
 */
...
void HAL_FDCAN_MspInit(FDCAN_HandleTypeDef* fdcanHandle) {
    ...
    if (0) {  // placeholder for if/else-if when no FDCAN is defined
    }
#ifdef FDCAN1
...
void board_fdcan2_init(void) {
    ...
    hfdcan2->Init = fdcan_init_cfg;
    can_fd_init(hfdcan2);  // filter derived from can2_recv_entries.h
...
void board_fdcan3_init(void) {
    ...
    hfdcan3->Init = fdcan_init_cfg;
    can_fd_init(hfdcan3);  // filter derived from can3_recv_entries.h
 ...
 * are app-specific and pins are Hi-Z at reset). CMSIS instance macros scope each
 * block to parts that physically have the ADC (e.g. g431 has ADC1/2 only).
 */
...
void board_init(void) {
    board_fdcan1_init();
    /* ponytail: g491vet routes FDCAN1 today. When FDCAN2/FDCAN3 are wired on the
       board, call board_fdcan2_init()/board_fdcan3_init() here. */
}
