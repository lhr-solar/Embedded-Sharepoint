...
StackType_t task_1_stack[configMINIMAL_STACK_SIZE];

StaticTask_t task_2_buffer;
StackType_t task_2_stack[configMINIMAL_STACK_SIZE];

StaticTask_t rx_task_buffer;
StackType_t rx_task_stack[configMINIMAL_STACK_SIZE];
...
static void rx_task(void *pvParameters) {
    ...
    can_status_t setInitStatus = can_register_id_set(hcan1, &canQueueSetStruct);
    ...
    while(1){
      ...
      if (can_recv_set(hcan1, &canQueueSetStruct, &id, portMAX_DELAY) == CAN_OK)
      {
          can_recv(hcan1, id, &fdcan1_rx_header, can1_rx_data, 0);
