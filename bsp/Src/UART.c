...
static uint8_t uart4_rx_queue_storage[UART4_RX_QUEUE_SIZE *
                                      sizeof(rx_payload_t)];  // Will be allocated based on
                                                              // queue_size in uart_init
...
static uint8_t
    uart5_rx_queue_storage[UART5_RX_QUEUE_SIZE *
                           sizeof(rx_payload_t)];  // Will be allocated based on queue_size
...
static uint8_t usart1_rx_queue_storage[USART1_RX_QUEUE_SIZE * sizeof(rx_payload_t)];
...
static uint8_t usart2_rx_queue_storage[USART2_RX_QUEUE_SIZE * sizeof(rx_payload_t)];
...
static uint8_t usart3_rx_queue_storage[USART3_RX_QUEUE_SIZE * sizeof(rx_payload_t)];
...
uart_status_t uart_init(UART_HandleTypeDef* handle) {
    ...
    if (handle->Instance == UART4) {
        // Create TX queue
        uart4_tx_queue = xQueueCreateStatic(UART4_TX_QUEUE_SIZE, sizeof(tx_payload_t),
                                            uart4_tx_queue_storage, &uart4_tx_queue_buffer);

        // Create RX queue
        uart4_rx_queue = xQueueCreateStatic(UART4_RX_QUEUE_SIZE, sizeof(rx_payload_t),
                                            uart4_rx_queue_storage, &uart4_rx_queue_buffer);
    ...
    if (handle->Instance == UART5) {
        ...
        uart5_tx_queue = xQueueCreateStatic(UART5_TX_QUEUE_SIZE, sizeof(tx_payload_t),
                                            uart5_tx_queue_storage, &uart5_tx_queue_buffer);

        // Create RX queue
        uart5_rx_queue = xQueueCreateStatic(UART5_RX_QUEUE_SIZE, sizeof(rx_payload_t),
                                            uart5_rx_queue_storage, &uart5_rx_queue_buffer);
    ...
    if (handle->Instance == USART1) {
        // Allocate static storage for TX queue
        usart1_tx_queue = xQueueCreateStatic(USART1_TX_QUEUE_SIZE, sizeof(tx_payload_t),
                                             usart1_tx_queue_storage, &usart1_tx_queue_buffer);

        // Create RX queue
        usart1_rx_queue = xQueueCreateStatic(USART1_RX_QUEUE_SIZE, sizeof(rx_payload_t),
                                             usart1_rx_queue_storage, &usart1_rx_queue_buffer);
    ...
    if (handle->Instance == USART2) {
        // Allocate static storage for TX queue
        usart2_tx_queue = xQueueCreateStatic(USART2_TX_QUEUE_SIZE, sizeof(tx_payload_t),
                                             usart2_tx_queue_storage, &usart2_tx_queue_buffer);

        // Create RX queue
        usart2_rx_queue = xQueueCreateStatic(USART2_RX_QUEUE_SIZE, sizeof(rx_payload_t),
                                             usart2_rx_queue_storage, &usart2_rx_queue_buffer);
    ...
    if (handle->Instance == USART3) {
        // Allocate static storage for TX queue
        usart3_tx_queue = xQueueCreateStatic(USART3_TX_QUEUE_SIZE, sizeof(tx_payload_t),
                                             usart3_tx_queue_storage, &usart3_tx_queue_buffer);

        // Create RX queue
        usart3_rx_queue = xQueueCreateStatic(USART3_RX_QUEUE_SIZE, sizeof(rx_payload_t),
                                             usart3_rx_queue_storage, &usart3_rx_queue_buffer);
    ...
    if (handle->Instance == LPUART1) {
        lpuart1_tx_queue = xQueueCreateStatic(LPUART1_TX_QUEUE_SIZE, sizeof(tx_payload_t),
                                              lpuart1_tx_queue_storage, &lpuart1_tx_queue_buffer);

        lpuart1_rx_queue = xQueueCreateStatic(LPUART1_RX_QUEUE_SIZE, sizeof(rx_payload_t),
                                              lpuart1_rx_queue_storage, &lpuart1_rx_queue_buffer);
...
/**
 * @brief Transmits data over UART. If transmission is in progress, data will be queued in internal
 * TX queue.
...
uart_status_t uart_send(UART_HandleTypeDef* handle, const uint8_t* data, uint8_t length,
    ...
    if (length == 0 ||
        !is_uart_initialized(handle)) {  // check if UART is initialized and data length is not 0
        return UART_ERR;
    }
    ...
    portENTER_CRITICAL();
    // check if UART is ready and queue is empty
    ...
    for (uint8_t i = 0; i < length; i += DATA_SIZE) {
        ...
        // EX: i=4, length=6, DataSize=4, then chunk_size = 2, instead of usual 4 since we've
        // reached end of length

        // Copy the appropriate number of bytes to the payload data
        memcpy(payload.data, &data[i],
               chunk_size);  // Usually chunk_size = DATA_SIZE until end of data length

        // If data size is smaller than DATA_SIZE, fill the rest of the payload
        // Fill the rest with 0 (or other padding if needed)
        if (chunk_size < DATA_SIZE) {
            memset(&payload.data[chunk_size], 0, DATA_SIZE - chunk_size);
        }
...
uart_status_t uart_recv(UART_HandleTypeDef* handle, uint8_t* data, uint8_t length,
                        TickType_t delay_ticks) {
    // check if data is not null, length is not 0 and UART is initialized
    ...
    while (bytes_received < length) {
        ...
        uint8_t copy_length =
            (length - bytes_received) >= DATA_SIZE ? DATA_SIZE : (length - bytes_received);
...
void HAL_UART_TxCpltCallback(UART_HandleTypeDef* huart) {
    ...
    tx_payload_t payload;
    while (count + DATA_SIZE <= sizeof(tx_buffer) &&
           xQueueReceiveFromISR(*tx_queue, &payload, &higherPriorityTaskWoken) == pdTRUE) {
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef* huart) {
    ...
    // Send data from &receivedData(pRxBuffPtr) to rx_queue
    xQueueSendFromISR(*rx_queue, &receivedData, &higherPriorityTaskWoken);

    // Trigger the next interrupt
    // pRxBufferPtr is a pointer to the buffer that will store the received data
    HAL_UART_Receive_IT(huart, rx_buffer, DATA_SIZE);
...
#ifdef USART1
void USART1_IRQHandler(void) { HAL_UART_IRQHandler(husart1); }
#endif /* USART1 */

#ifdef USART2
void USART2_IRQHandler(void) { HAL_UART_IRQHandler(husart2); }
#endif /* USART2 */

#ifdef USART3
void USART3_IRQHandler(void) { HAL_UART_IRQHandler(husart3); }
#endif /* USART3 */
