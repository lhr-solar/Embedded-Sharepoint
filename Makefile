...
test:
ifdef test
	$(MAKE) -C BSP -C STM32L431 -j TARGET=bsp PROJECT_DIR=../.. BUILD_DIR=../../Objects TEST=../Tests/$(test).c
else
...
psom_test:
ifdef test
	$(MAKE) -C BSP -C STM32L431 -j TARGET=bsp PROJECT_DIR=../.. BUILD_DIR=../../Objects TEST=../../Tests/PeripheralSOM/$(test).c
else
	$(error test is not set (e.g. make psom_test test=HelloWorld))
endif
